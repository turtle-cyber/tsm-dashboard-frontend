...
import {
  mockThreatsList,
  threatTimeRanges,
  threatSeverityFilters,
} from "@/data/mockThreatsListData";
...
export default function ThreatsList() {
  ...
  const filteredThreats = mockThreatsList.filter((threat) => {
    const matchesSearch =
      ...
      threat.mitreTechniques.some((tech) =>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Security threats detected across your environment
          </p>
        </div>
      </div>
              ...
              <div className="flex gap-1">
                {threatSeverityFilters.map((filter) => (
                  <Button
                    key={filter.value}
                    variant={
                      selectedSeverity === filter.value ? "default" : "outline"
                    }
                    size="sm"
              ...
              <TableBody>
                {filteredThreats.map((threat) => (
                  ...
                  >
                    ...
                    <TableCell>
                      <Badge
                        className={`text-xs ${getVerdictColor(
                          threat.analystVerdict
                        )}`}
                      >
